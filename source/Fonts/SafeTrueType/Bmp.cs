...
namespace SafeStbTrueTypeSharp
{
	...
	internal class Bitmap
	{
		...
		{
			var active = new SimpleHolder<stbtt__active_edge>(); // fixed
			var y = 0;
			var j = 0;
			var i = 0;
			float[] scanline;
			...
			int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, int invert)
		{
			...
			var winding_count = 0;
			int[]? winding_lengths = null; // fixed
			var windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, out winding_lengths,
				out winding_count);
			if (windings != null)
				stbtt__rasterize(windings, winding_lengths!, winding_count, scale_x, scale_y, shift_x, shift_y, // fixed
                    x_off, y_off, invert);
		}
	}
}