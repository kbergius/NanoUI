...
namespace SafeStbTrueTypeSharp
{
    // fixed nullable warnings
    internal class FontInfo
	{
		public Buf? cff = null; // fixed
		public Buf? charstrings = null; // fixed
        public FakePtr<byte> data;
		public Buf? fdselect = null; // fixed
        public Buf? fontdicts = null; // fixed
        public int fontstart;
		public int glyf;
		public int gpos;
		public Buf? gsubrs = null; // fixed
        public int head;
		public int hhea;
		public int hmtx;
		...
		public int stbtt_InitFont_internal(byte[] data, int fontstart)
		{
			...
			{
				Buf? b = null; // fixed
                Buf? topdict = null; // fixed
                Buf? topdictidx = null; // fixed
                var cstype = (uint)2;
		...
		public int stbtt_GetCodepointShape(int unicode_codepoint, out stbtt_vertex[]? vertices) // fixed
        {
		...
		public int stbtt__GetGlyphShapeTT(int glyph_index, out stbtt_vertex[]? pvertices) // fixed
        {
			short numberOfContours = 0;
			FakePtr<byte> endPtsOfContours;
			var data = this.data;
			stbtt_vertex[]? vertices = null; // fixed
            var num_vertices = 0;
			var g = stbtt__GetGlyfOffset(glyph_index);
			pvertices = null;
			if (g < 0)
			...
			else if (numberOfContours < 0)
			{
				...
				while (more != 0)
				{
					...
					if (comp_num_verts > 0)
					{
						...
						if (num_vertices > 0)
							Array.Copy(vertices!, tmp, num_vertices); // fixed

                        Array.Copy(comp_verts!, 0, tmp, num_vertices, comp_num_verts); // fixed
                        vertices = tmp;
		...
		public Buf stbtt__cid_get_glyph_subrs(int glyph_index)
		{
			...
			var fdselector = -1;
			var i = 0;
			fdselect!.stbtt__buf_seek(0); // fixed
			...
			return Buf.stbtt__get_subrs(this.cff!, fontdicts!.stbtt__cff_index_get(fdselector)); // fixed
        }
		...
		public int stbtt__run_charstring(int glyph_index, CharStringContext c)
		{
			...
			for (i = 0; i < subr_stack.Length; ++i)
				subr_stack[i] = null!; // fixed

            var subrs = this.subrs;
			float f = 0;
			var b = this.charstrings!.stbtt__cff_index_get(glyph_index); // fixed
            while (b.cursor < b.size)
			{
				...
				switch (b0)
				{
					...
					case 0x1D:
						...
						b = b0 == 0x0A ? subrs!.stbtt__get_subr(v) : this.gsubrs!.stbtt__get_subr(v); // fixed
                        if (b.size == 0)
							return 0;
						b.cursor = 0;
						clear_stack = 0;
		...
		public int stbtt__GetGlyphShapeT2(int glyph_index, out stbtt_vertex[]? pvertices) // fixed
        {
		...
		public int stbtt_GetGlyphShape(int glyph_index, out stbtt_vertex[]? pvertices) // fixed
        {
			if (this.cff!.size == 0) // fixed
			...
			float shift_x, float shift_y, int glyph, ref int width, ref int height, ref int xoff, ref int yoff)
		{
			...
			if (gbm.w != 0 && gbm.h != 0)
			{
				...
				gbm.stride = gbm.w;
				gbm.stbtt_Rasterize(0.35f, vertices!, num_verts, scale_x, scale_y, shift_x, shift_y, ix0, iy0, 1); // fixed
            }
		...
		public byte[]? stbtt_GetGlyphSDF(float scale, int glyph, int padding, // fixed
			...
			scale_y = -scale_y;
			{
				...
				for (y = iy0; y < iy1; ++y)
					for (x = ix0; x < ix1; ++x)
					{
						...
						var winding = stbtt__compute_crossings_x(x_gspace, y_gspace, num_verts, verts!); // fixed
                        for (i = 0; i < num_verts; ++i)
						{
							var x0 = verts![i].x * scale_x; // fixed
                            var y0 = verts[i].y * scale_y;
		...
		public byte[]? stbtt_GetCodepointSDF(float scale, int codepoint, int padding, // fixed
            byte onedge_value, float pixel_dist_scale, ref int width, ref int height, ref int xoff, ref int yoff)
		{
