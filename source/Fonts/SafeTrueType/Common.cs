...
using static SafeStbTrueTypeSharp.PackContext;
...
namespace SafeStbTrueTypeSharp
{
    ...
    internal static partial class Common
	{
		...
		public class stbtt__active_edge
		{
			...
			public float fdy;
			public float fx;
			public stbtt__active_edge? next; // fixed
			public float sy;
		}
		...
		public static stbtt__point[]? stbtt_FlattenCurves(stbtt_vertex[] vertices, int num_verts,  // fixed
            float objspace_flatness, out int[]? contour_lengths, out int num_contours) // fixed
		{
			stbtt__point[]? points = null; // fixed
			var num_points = 0;
				...
				for (i = 0; i < num_verts; ++i)
					switch (vertices[i].type)
					{
						case STBTT_vmove:
							...
							x = vertices[i].x;
							y = vertices[i].y;
							stbtt__add_point(points!, num_points++, x, y); // fixed
							break;
						case STBTT_vline:
							x = vertices[i].x;
							y = vertices[i].y;
						...
						case STBTT_vcurve:
							stbtt__tesselate_curve(points!, ref num_points, x, y, vertices[i].cx, vertices[i].cy,
								vertices[i].x, vertices[i].y, objspace_flatness_squared, 0); // fixed
                            x = vertices[i].x;
							y = vertices[i].y;
							break;
						case STBTT_vcubic:
							stbtt__tesselate_cubic(points!, ref num_points, x, y, vertices[i].cx, vertices[i].cy,
								...
								objspace_flatness_squared, 0); // fixed
                            x = vertices[i].x;
							y = vertices[i].y;
							break;
					}
