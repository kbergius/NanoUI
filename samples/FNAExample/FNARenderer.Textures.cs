...
namespace FNAExample
{
    ...
    public partial class FNARenderer
    {
        ...
        public int CreateTexture(string path, TextureFlags textureFlags = 0)
        {
            ...
            {
                _textures[_texCounter] = Texture2D.FromStream(_device, stream);
            }
            ...
            if (!data.IsEmpty && _textures.TryGetValue(texture, out var tex))
            {
                switch (tex.Format)
                {
                    case SurfaceFormat.Alpha8:
                        // NanoUI's font manager sends data in 8-bit format,
                        // when it wants to update font atlas texture.
                        // In this renderer surface format is configured to use
                        // Alpha8 (unsigned A 8-bit - 8 bits in alpha channel).
                        // Note: in shader we use texture.w (alpha).
        ...
        internal static SurfaceFormat MapTextureFormat(TextureFormat format)
        {
            ...
            {
                TextureFormat.R => SurfaceFormat.Alpha8,   // Unsigned A 8-bit - 8 bits in alpha channel.
