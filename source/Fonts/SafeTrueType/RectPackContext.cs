...
namespace SafeStbTrueTypeSharp
{
    ...
    internal class PackContext
	{
		...
		public int height;
		public stbrp_context? pack_info; // fixed
		public int padding;
		...
		{
			public int[]? array_of_unicode_codepoints; // fixed
			public stbtt_packedchar[]? chardata_for_range;  // fixed
			...
			for (i = 0; i < num_ranges; ++i)
			{
				...
				for (j = 0; j < ranges[i].num_chars; ++j)
				{
					...
					var codepoint = ranges[i].array_of_unicode_codepoints == null
						...
						: ranges[i].array_of_unicode_codepoints![j]; // fixed
			...
			FakePtr<stbtt_pack_range> ranges, int num_ranges, stbrp_rect[] rects)
		{
			...
			for (i = 0; i < num_ranges; ++i)
			{
				...
				for (j = 0; j < ranges[i].num_chars; ++j)
				{
					...
					{
						var bc = ranges[i].chardata_for_range![j]; // fixed
						var advance = 0;
						var lsb = 0;
						var x0 = 0;
						var y0 = 0;
					...
					{
						ranges[i].chardata_for_range![j] = ranges[i].chardata_for_range![missing_glyph]; // fixed
					}
					else
					{
						return_value = 0;
			...
			for (i = 0; i < num_ranges; ++i)
				for (j = 0; j < ranges[i].num_chars; ++j)
					ranges[i].chardata_for_range![j].x0 = ranges[i].chardata_for_range![j].y0 =
						ranges[i].chardata_for_range![j].x1 = ranges[i].chardata_for_range![j].y1 = 0; // fixed
			n = 0;
			for (i = 0; i < num_ranges; ++i)
